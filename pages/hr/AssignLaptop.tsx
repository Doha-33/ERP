...
export const AssignLaptopPage: React.FC = () => {
  ...
  const { assignLaptops, addAssignLaptop, updateAssignLaptop, deleteAssignLaptop } = useData();
  ...
  const handleSave = (laptop: AssignLaptop) => {
    if (editingLaptop) updateAssignLaptop(laptop);
    else addAssignLaptop(laptop);
  };
  ...
  const confirmDelete = useCallback(() => {
    if (deleteId) {
      deleteAssignLaptop(deleteId);
      setDeleteId(null);
    }
  }, [deleteId, deleteAssignLaptop]);

  // Extract unique employee names for the dropdown
  const uniqueEmployeeNames = useMemo(() => {
    const names = assignLaptops.map(l => l.empName).filter(Boolean);
  ...
  const filtered = useMemo(() => {
    return assignLaptops.filter(l => {
      const matchesSearch = (l.empName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
                           (l.serialNumber || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
                           (l.empCode || '').toLowerCase().includes(searchTerm.toLowerCase());
      const matchesSelect = selectedEmployee === '' || l.empName === selectedEmployee;
      return matchesSearch && matchesSelect;
    });
  }, [assignLaptops, searchTerm, selectedEmployee]);
    ...
    { 
      ...
      render: (l) => (
        <div className="flex items-center gap-3">
           ...
           <span className="font-bold text-gray-900 dark:text-white">{l.empName}</span>
        </div>
      )
    },
    { header: t('device_type'), accessorKey: 'deviceType', className: 'text-gray-500 font-medium' },
    { header: t('serial_number'), accessorKey: 'serialNumber', className: 'text-gray-500 font-mono text-xs' },
    ...
    { 
      ...
      render: (l) => (
        <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase border ${
            l.status === 'Done' ? 'bg-green-50 text-green-600 border-green-100 dark:bg-green-900/20 dark:border-green-900/30' : 
    ...
    },
    {
      ...
      render: (l) => (
        <div className="flex items-center justify-center gap-2">
           <button onClick={() => handleEdit(l)} className="p-1.5 text-gray-500 hover:text-primary hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded border border-gray-200 dark:border-gray-700 transition-colors"><Edit2 size={14} /></button>
           <button onClick={() => handleDelete(l.id)} className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded border border-gray-200 dark:border-gray-700 transition-colors"><Trash2 size={14} /></button>
        </div>
      )
    }
       ...
       <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
           <div className="flex items-center gap-2 text-sm text-gray-400 font-medium mb-1 uppercase tracking-wider">
              ...
              <span className="text-primary">{t('assign_laptop')}</span>
           </div>
           <h1 className="text-2xl font-bold dark:text-white">{t('manage_assign_laptop')}</h1>
        </div>
        <div className="flex gap-3">
           <ExportDropdown data={filtered} filename="assigned_devices" />
           <Button onClick={() => { setEditingLaptop(null); setIsModalOpen(true); }} className="bg-[#4361EE] hover:bg-blue-700 shadow-lg shadow-primary/20">
              <Plus size={18} /> {t('add_assign_laptop')}
           </Button>
        </div>
      </div>
      ...
      <Card className="min-h-[500px] border-none shadow-xl shadow-gray-200/50 dark:shadow-none">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 bg-gray-50 dark:bg-gray-800/30 p-4 rounded-2xl">
           <div className="w-full md:w-80 relative">
              <Input 
                placeholder="Search by name, code or SN..."
                value={searchTerm}
        ...
        <Table 
           ...
           emptyMessage="No device records found matching your selection"
        />
      </Card>
