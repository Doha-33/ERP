...
import { 
  ...
  Users, UserCheck, UserX, Mail, Phone, Calendar, Building2, Briefcase
} from 'lucide-react';
import { Card, Button, Input, Badge, ExportDropdown, StatCard } from '../../components/ui/Common';
...
export const Employees: React.FC = () => {
  ...
  const handleSave = async (employeeData: Partial<Employee>) => {
    try {
      setIsLoading(true);
      if (editingEmployee) {
        await updateEmployee({ ...employeeData, id: editingEmployee.id } as Employee);
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      toast.success(t('employees_deleted_successfully', { count: selectedIds.length }));
      setSelectedIds([]);
  ...
  const activeEmployees = accessibleEmployees.filter(e => e.employeeStatus === 'ACTIVE').length;
  const inactiveEmployees = accessibleEmployees.filter(e => e.employeeStatus !== 'ACTIVE').length;
  ...
  const columns: Column<Employee>[] = useMemo(() => [
    {
      ...
      render: (emp) => (
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-sm font-medium text-gray-900">{emp.employeeCode || '-'}</span>
        </div>
      )
    },
    {
      ...
      render: (emp) => (
        <div className="flex flex-col">
          ...
          <span className="text-xs text-gray-500">{emp.jobId?.jobName || '-'}</span>
        </div>
      )
    },
    {
      ...
      render: (emp) => (
        <div className="flex flex-col gap-0.5">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-gray-600">{emp.phoneNumber || '-'}</span>
          </div>
        </div>
      )
    },
      ...
      render: (emp) => (
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-gray-600">{emp.companyId?.name || '-'}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Briefcase size={12} className="text-gray-400" />
            <span className="text-xs text-gray-600">{emp.branchId?.name || '-'}</span>
          </div>
        </div>
      )
    },
    {
      ...
      render: (emp) => (
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-sm text-gray-600">{emp.hireDate ? new Date(emp.hireDate).toLocaleDateString() : '-'}</span>
        </div>
      )
    },
    {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-gray-500">{t('inactive_employees')}</p>
                <p className="text-xl font-bold text-gray-900">{inactiveEmployees}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <ConfirmationModal
        ...
        message={t('are_you_sure_delete_employees', { count: selectedIds.length })}
      />
    </div>
  );
};