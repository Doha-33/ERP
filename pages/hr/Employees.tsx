...
export const Employees: React.FC = () => {
  ...
  const columns: Column<any>[] = useMemo(() => [
    ...
    { 
      ...
      render: (emp) => (
        <div className="flex items-center gap-3">
           <img src={emp.photo || `https://ui-avatars.com/api/?name=${emp.fullName}&background=random`} alt={emp.fullName} className="w-9 h-9 rounded-full object-cover" />
    ...
    { 
      header: t('state'), 
      render: (emp) => (
        <Badge status={emp.employeeStatus || 'Active'}>{t((emp.employeeStatus || 'Active').toLowerCase().replace(/\s+/g, '_'))}</Badge>
      )
    },
    {
      header: t('actions'),
      ...
      render: (emp) => (
        <div className="flex items-center justify-center gap-2">
           <button onClick={() => handleView(emp.id)} className="p-1.5 text-gray-500 hover:text-primary hover:bg-gray-100 dark:hover:bg-gray-800 rounded transition-colors">
              <Eye size={16} />
           </button>
           {isAdmin && (
             <>
               ...
               <button onClick={() => handleDelete(emp.id)} className="p-1.5 text-gray-500 hover:text-red-500 hover:bg-gray-100 dark:hover:bg-gray-800 rounded transition-colors">
                  <Trash2 size={16} />
               </button>
             </>
           )}
        ...
        {viewMode === 'list' ? (
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filtered.map((emp) => {
                 ...
                 return (
                 <div key={emp.id} className={`bg-[#f0f7ff] dark:bg-gray-800/50 rounded-2xl p-6 flex flex-col items-center text-center relative group border-2 transition-all ${isSelected ? 'border-primary shadow-lg ring-4 ring-primary/10' : 'border-transparent hover:border-blue-200'}`}>
                    {isAdmin && (
                      <div className="absolute top-4 left-4">
                        <input 
                          ...
                          checked={isSelected}
                          onChange={() => {
                            if (isSelected) setSelectedIds(prev => prev.filter(i => i !== emp.id));
                            else setSelectedIds(prev => [...prev, emp.id]);
                          }}
                    ...
                    {isAdmin && (
                      <div className="absolute top-4 right-4">
                        <Dropdown trigger={<button className="p-1 text-gray-400 hover:text-primary"><MoreHorizontal size={20} /></button>} items={[{ label: t('edit_employee'), icon: <Edit2 size={16}/>, onClick: () => handleEdit(emp) }, { label: t('delete'), icon: <Trash2 size={16}/>, variant: 'danger', onClick: () => handleDelete(emp.id) }]} />
                      </div>
                    )}
                    ...
                    <div className="w-20 h-20 rounded-2xl bg-primary flex items-center justify-center mb-5 shadow-lg shadow-primary/20 overflow-hidden border-4 border-white dark:border-gray-700">
                      <img src={emp.photo || `https://ui-avatars.com/api/?name=${emp.fullName}&background=random`} alt={emp.fullName} className="w-full h-full object-cover" />
                    </div>
                    ...
                    <Button onClick={() => handleView(emp.id)} variant="outline" size="sm" fullWidth className="bg-white dark:bg-dark-surface font-bold">View Profile</Button>
                 </div>
              );})}
           </div>
        )}
