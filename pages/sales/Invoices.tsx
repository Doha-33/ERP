...
import { Plus, Search, Edit2, Trash2, Eye, Filter, X, Calendar, DollarSign, CreditCard, Warehouse, FileText } from "lucide-react";
...
export const Invoices: React.FC = () => {
  ...
  const handleSave = async (invoice: Partial<SalesInvoice>) => {
    try {
      setIsLoading(true);
      if (editingInvoice) {
        await updateSalesInvoice({ ...invoice, _id: editingInvoice._id, id: editingInvoice.id } as SalesInvoice);
        toast.success(t("invoice_updated_successfully"));
      } else {
        await addSalesInvoice(invoice as SalesInvoice);
      ...
      toast.error(t("failed_to_save_invoice"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      toast.success(t("invoices_deleted_successfully", { count: selectedIds.length }));
      setSelectedIds([]);
  ...
  const getCustomerName = (invoice: SalesInvoice): string => {
    if (typeof invoice.customerId === "object" && invoice.customerId !== null) {
      return (invoice.customerId as any)?.customerName || (invoice.customerId as any)?.name || "-";
    }
    return "-";
  };

  const getWarehouseName = (invoice: SalesInvoice): string => {
    if (typeof invoice.warehouseId === "object" && invoice.warehouseId !== null) {
  ...
  const getOrderNo = (invoice: SalesInvoice): string => {
    if (typeof invoice.salesOrderId === "object" && invoice.salesOrderId !== null) {
  ...
  const partiallyPaid = filteredInvoices.filter(i => i.paymentStatus === "PARTIALLY_PAID").length;
  ...
  const columns: Column<SalesInvoice>[] = useMemo(
    () => [
        ...
        render: (i) => (
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="font-medium text-gray-900">{i.invoiceNumber}</span>
              <span className="text-xs text-gray-500">{t("order")}: {getOrderNo(i)}</span>
            </div>
          </div>
        )
      },
        ...
        render: (i) => (
          <div className="flex flex-col gap-0.5">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-gray-600">{t("issued")}: {formatDate(i.issuedDate)}</span>
            </div>
            {i.dueDate && (
              <div className="flex items-center gap-1.5">
                <Calendar size={12} className="text-gray-400" />
                <span className="text-xs text-gray-600">{t("due")}: {formatDate(i.dueDate)}</span>
              </div>
            )}
          </div>
        )
      },
      {
        ...
        render: (i) => (
          <div className="flex flex-col">
            <span className="font-bold text-indigo-600">{i.totalAmount?.toLocaleString()} EGP</span>
            <span className="text-xs text-gray-400">{t("tax")}: {i.taxAmount?.toLocaleString()}</span>
          </div>
        )
      },
      {
      ...
      },
      {
        ...
        render: (i) => (
          <div className="flex items-center justify-center gap-2">
            <button
              onClick={() => navigate(`/sales/invoices/${i._id || i.id}`)}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-3 shadow-sm">
          ...
          <p className="text-xl font-bold text-orange-600 mt-1">{partiallyPaid}</p>
        </div>
      </div>
      ...
      <ConfirmationModal
        ...
        message={t("are_you_sure_delete_invoices", { count: selectedIds.length })}
      />
    </div>
  );
};