...
export const Income: React.FC = () => {
  ...
  const [records] = useState<IncomeRecord[]>([
    {
      id: 'IN849',
      date: '3/12/2033',
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          <ExportDropdown data={records} filename="income" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
        <StatCard title={t('total_income_this_month')} value="$25,430" icon={<DollarSign size={20} />} color="blue" />
        <StatCard title={t('paid_income')} value="$22,230" icon={<CreditCard size={20} />} color="green" />
        ...
        <StatCard title={t('top_customer')} value="ABC Company" icon={<Users size={20} />} color="blue" />
      </div>
      ...
      <Modal 
        ...
        title={<div className="flex items-center gap-2"><Plus size={20} className="text-primary" /> {t('add_income')}</div>}
      >
        <div className="grid grid-cols-2 gap-4 mt-4">
          <Input label="ID" placeholder="32243" required />
          <Input label={t('date')} type="date" required />
          <Input label={t('source')} placeholder="Company x" required />
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <Button onClick={() => setIsAddModalOpen(false)}>{t('add_income')}</Button>
        </div>
      </Modal>
      ...
      <Modal 
        ...
        title={<div className="flex items-center gap-2"><Edit2 size={20} className="text-primary" /> {t('edit_income')}</div>}
      >
        <div className="grid grid-cols-2 gap-4 mt-4">
          ...
          <Input label={t('source')} defaultValue={selectedRecord?.source} required />
