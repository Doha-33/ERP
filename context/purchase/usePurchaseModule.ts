...
export const usePurchaseModule = (fetchAllData?: () => Promise<void>) => {
  ...
  const [returnsToSupplier, setReturnsToSupplier] = useState<
    ReturnToSupplier[]
  >([]);
  ...
  const fetchGoodsReceipts = useCallback(async () => {
    try {
      const data = await purchaseService.getGoodsReceipts();
      setGoodsReceipts(Array.isArray(data) ? data : data?.data || []);
    } catch (error) {
      console.error("Error fetching Goods Receipts:", error);
    }
  }, []);

  const addGoodsReceipt = useCallback(
  ...
  const fetchPurchaseRequests = useCallback(async () => {
    try {
      ...
      setPurchaseRequests(Array.isArray(data) ? data : data?.data || []);
    } catch (error) {
  ...
  const addSupplierRating = useCallback(
    async (rating: SupplierRating) => {
  ...
  const updateSupplierRating = useCallback(
    async (rating: SupplierRating) => {
  ...
  const fetchSupplierRatings= useCallback(async () => {
    try {
      const data = await purchaseService.getSupplierRatings();
      setSupplierRatings(Array.isArray(data) ? data : data?.data || []);
    } catch (error) {
      console.error("Error fetching Supplier Ratings:", error);
    }
  }, []);

  const deleteSupplierRating = useCallback(
    async (id: string) => {
  ...
  const addReturnToSupplier = useCallback(
    async (ret: ReturnToSupplier) => {
  ...
  const fetchReturnsToSupplier = useCallback(async () => {
    try {
      const data = await purchaseService.getReturnsToSupplier();
      setReturnsToSupplier(Array.isArray(data) ? data : data?.data || []);
    } catch (error) {
      console.error("Error fetching Returns To Supplier:", error);
    }
  }, []);

  const deleteReturnToSupplier = useCallback(
    async (id: string) => {
  ...
  const fetchPurchaseData = useCallback(async () => {
    try {
      ...
      setPurchaseOrders(
        Array.isArray(orderRes) ? orderRes : orderRes?.data || [],
      );
      setPurchaseRequests(Array.isArray(reqRes) ? reqRes : reqRes?.data || []);
      setPurchaseInvoices(
        Array.isArray(invoiceRes) ? invoiceRes : invoiceRes?.data || [],
      );
      setGoodsReceipts(
        Array.isArray(receiptRes) ? receiptRes : receiptRes?.data || [],
      );
      setSupplierRatings(
        Array.isArray(supplierRes) ? supplierRes : supplierRes?.data || [],
      );
      setReturnsToSupplier(
        Array.isArray(returnsRes) ? returnsRes : returnsRes?.data || [],
      );
      setPurchaseReports(
        Array.isArray(reportRes) ? reportRes : reportRes?.data || [],
      );
    } catch (error) {
