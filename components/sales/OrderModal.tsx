...
export const OrderModal: React.FC<OrderModalProps> = ({
  ...
  const { customers, companies, branches, warehouses, employees, products } =
    useData();
  ...
  const filteredWarehouses = useMemo(() => {
    ...
    return warehouses.filter(warehouse => {
      const warehouseBranchId = typeof warehouse.branchId === "object"
        ? (warehouse.branchId as any)?._id
        : warehouse.branchId;
      return warehouseBranchId === formData.branchId;
    });
  }, [warehouses, formData.branchId]);

  // Filter salespersons based on selected branch (optional)
  const filteredSalespersons = useMemo(() => {
    if (!formData.branchId) return employees;
    return employees.filter(emp => {
      const empBranchId = typeof emp.branchId === "object"
        ? (emp.branchId as any)?._id
        : emp.branchId;
      return empBranchId === formData.branchId;
    });
  }, [employees, formData.branchId]);
  ...
  useEffect(() => {
    ...
    const transformItems = (items: any[] = []) => {
      ...
      return items.map((item) => ({
        ...
        tax: item.tax || 0,
        total: item.total || (item.quantity * item.unitPrice - item.discount + item.tax),
      }));
    };
    ...
    if (orderToEdit) {
      setFormData({
        ...
        customerId: extractId(orderToEdit.customerId),
        companyId: extractId(orderToEdit.companyId),
        branchId: extractId(orderToEdit.branchId),
        warehouseId: extractId(orderToEdit.warehouseId),
        salespersonId: extractId(orderToEdit.salespersonId),
      ...
      setItems(transformItems(orderToEdit.items));
    } else {
      setFormData({
        orderNo: "",
        customerId: "",
  ...
  const salespersonOptions = filteredSalespersons.map((e) => ({
    value: e._id || e.id,
    label: e.fullName,
  }));
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = error?.response?.data?.message || error?.message || "Failed to save order";
      toast.error(message);
    } finally {
